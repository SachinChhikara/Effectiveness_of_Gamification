...
if (is_column_of_type(analysis_data, "Group", "character")) {
  "The numeric values in this column had their numeric values converted to their character represntation"
}else{
  "The numeric values have not been converted, please makes sure to run the 02_data_cleaning.R"
}


check_gender_values(analysis_data, "gender", c("male", "female"))

check_numeric_columns(analysis_data, c("WordProcPretest", "SpreadsheetsPretest", 
                                       "PresentationPretest", "DatabasesPretest"))

check_numeric_columns(analysis_data, c("WordProcPosttest", "SpreadsheetsPosttest", 
                                       "PresentationPosttest", "DatabasesPosttest"))
...
if (length(analysis_data) == 11){
  ...
  "Error: There are more or less variables that expected, please examine the dataset "
}
